...
# Função de Treino
def train_model(model, train_loader, optimizer, criterion=nn.CrossEntropyLoss(), device=torch.device("cuda")):
    model.train()
    running_loss = 0.0
...
def test_model(model, test_loader, zero_devision_value=0, device=torch.device("cuda")):
    ...
    precision = precision_score(all_labels, all_predictions, average='macro', zero_division=zero_devision_value)
    recall = recall_score(all_labels, all_predictions, average='macro', zero_division=zero_devision_value)
    f1 = f1_score(all_labels, all_predictions, average='macro', zero_division=zero_devision_value)
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(p=0.25), # Invertendo a imagem da esquerda para a direita com probabilidade p 25%
        transforms.RandomVerticalFlip(p=0.25) # Invertendo a imagem de cima para baixo com probabilidade p 25%
    ])
    ...
    for epoch in range(epochs):
        ...
        accuracy, precision, recall, f1 = test_model(model, test_loader, zero_devision_value=1) 
        
        print(f"Época [{epoch+1}/{epochs}]")
        print(f"Loss de Treinamento: {train_loss:.4f}")
        print(f"Métricas calculadas no conjunto de Teste:")
        print(f"Acurácia: {accuracy:.3f}, Precisão: {precision:.3f}, Recall: {recall:.3f}, F1-Score: {f1:.3f}\n")
