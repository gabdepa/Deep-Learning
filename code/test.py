...
def test_model(
    ...
    test_transform = transforms.Compose(
        [
            ...
            transforms.Lambda(lambda x: enhance_colors(x)),  # Aplicar o realce de cores
            transforms.Lambda(
                lambda x: adaptive_histogram_equalization(x)
            ),  # Aplicar equalização
            transforms.Lambda(lambda x: rgb_to_lab_transform(x)),  # Conversão para Lab
